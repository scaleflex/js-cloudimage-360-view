...
export const getCurrentOriginalImage = (
  movingDirection,
  imagesX,
  imagesY,
  activeImageX,
  ...
  const originalImagesXSrcs = imagesX.map((image) =>
    image.src.replace(ORGINAL_SIZE_REGEX, '').replace(AND_SYMBOL_REGEX, '?')
  );

  const originalImagesYSrcs = imagesY.map((image) =>
    image.src.replace(ORGINAL_SIZE_REGEX, '').replace(AND_SYMBOL_REGEX, '?')
  );
