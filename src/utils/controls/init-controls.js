...
export const initControls = (controlsConfig, controlsTriggers) => {
  const { container, controlReverse, spinReverse, stopAtEdges } =
    controlsConfig;

  const { onRightStart, onLeftStart, onTopStart, onBottomStart, onEventEnd } =
    controlsTriggers;

  const controlElements = {};
  const isReverse = controlReverse ? !spinReverse : spinReverse;

  const left = container.querySelectorAll(
    '.cloudimage-360-left, .cloudimage-360-prev'
  )[0];

  const right = container.querySelectorAll(
